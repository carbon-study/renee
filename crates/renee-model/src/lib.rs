...
#![allow(
    ...
    reason = "the reference model mirrors canonical private storage encoding"
)]
...
impl UpdateModel {
    ...
    /// Enumerates the members of one captured acceptance window.
    ///
    /// The iterator order is a deterministic model convenience only. Enumeration
    /// delivery is semantically an unordered set and clients cannot rely on this order.
    pub fn enumerate(
        &self,
        document_id: DocumentId,
        ...
        self.acceptance_window(document_id, after, terminal_sequence)
    }
